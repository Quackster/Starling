...
/**
 * Maps room-domain models into room service contracts.
 */
public final class RoomContractMapper {

    /**
     * Creates a new RoomContractMapper.
     */
    private RoomContractMapper() {}
    ...
    public static RoomSnapshot toRoomSnapshot(Session session) {
        ...
        LoadedRoom<?> loadedRoom = RoomRegistryHolder.REGISTRY.find(presence.type(), presence.roomId());
     ...
     * @return the result of this operation
     */
    public static RoomVisuals toVisuals(RoomLayoutRegistry.RoomVisuals visuals) {
    ...
    private static void addOccupants(
        ...
        if (loadedRoom == null || loadedRoom.isEmpty()) {
            ...
            occupants = loadedRoom.getOccupantSnapshots().stream()
                    ...
                    .map(RoomContractMapper::toRoomOccupant)
                    .toList();
        }
    ...
    private static void addPublicRoomItems(RoomSnapshot.Builder builder, PublicRoomEntity room) {
        ...
        if (!items.isEmpty()) {
            builder.addAllPublicRoomItems(items.stream().map(RoomContractMapper::toPublicRoomItem).toList());
            return;
        }
    ...
    /**
     * Provides singleton room registry access without threading it through every mapper call.
     */
    private static final class RoomRegistryHolder {
        private static final org.starling.game.room.registry.RoomRegistry REGISTRY =
                org.starling.game.room.registry.RoomRegistry.getInstance();
    }
}
